...
# Создаём все таблицы при старте, если их ещё нет
...
# Создание учётной записи администратора по умолчанию (если нет)
db = DBSession()
...
# 2. Авторизация (логин) студента или админа по логину/паролю (JWT)
@app.post("/authorization", response_model=Token)
def authorize(data: LoginData):
    auth = authenticate_user(data.login, data.password)
    if not auth:
...
# 3. Добавление достижения (только для студента)
...
# 4. Просмотр достижений (для студента – свои, для админа – все)
...
def list_achievements(current_user=Depends(get_current_user)):
    ...
    if getattr(current_user, 'role', None) == "admin":
        records = db.query(Achievement).all()
    else:
        records = db.query(Achievement).filter(Achievement.student_id == current_user.id).all()
    db.close()
    return records

# Дополнительно: список всех студентов (только для админа)
